...
def leer_datos(archivo):
    ...
    try:
        with open(archivo, 'r') as file:
            for line in file:
                partes = line.strip().split()  # Ensure no trailing spaces
                if len(partes) == 2:
                    nuevas_horas.append(partes[1])  # Time (string)
                    nuevas_deformaciones.append(float(partes[0]))  # Deformation (float)
...
def actualiza_grafica(i, horas, deformaciones):
    # Read new data
    nuevas_horas, nuevas_deformaciones = leer_datos("datos.txt")
