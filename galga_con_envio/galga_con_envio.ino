...
#define SENSOR 35 //Pin del ADC, se usa este pq solo funciona como ADCpinMode(SENSOR,INPUT);
  ...
  */
  ...
  if (pin_correcto == false){
    Serial.println("El pin no es correcto, por favor elige otro");
      ...
      uint16_t lectura = analogRead(numero_pin); // Lee la tensión del pin. Como entero pq lee el valor analogico y lo convierte a digital (ADC)
      float tension = (lectura * 3.3)/4095.0; // Traduce el valor digital medido por el ADC del esp32 a su correspondiente analógico (lectura es a 4095 lo que x es a 3.3)
      return tension;
  }
}
...
void loop() {
  ...
  float deformacion = V2def(tension_medida, 5, 3.16);
  Serial.print("Deformación de: ");
  Serial.println(deformacion, 6);
  delay(500);

  snprintf(buffer, sizeof(buffer), "%f", deformacion); // Esto no imprime el valor, almacena en "buffer" la cadena de texto formateada. Es decir, en vez de imprimir una cadena de texto o lo que sea, la mete en buffer

  Serial.println("Intentando enviar el mensaje...");
  delay(500);

  if (client.publish("Medidas/Deformación", buffer)){
